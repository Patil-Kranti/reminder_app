...
class _TodoListState extends State<TodoList> {
  ...
  Future _getCategoryList() async {
    return _categoryProvider.getAllCategory().then(
      (categories) {
  ...
  Widget _createAppBar() {
    return new AppBar(
      ...
      leading: Padding(
        padding: const EdgeInsets.only(left: 18.0, top: 15.0, bottom: 15.0),
        child: Image(
          image: AssetImage('icon/top_bar.png'),
          width: 20.0,
        ),
      ),
  ...
  void _getCategoryTodo() {
    _filterByCategory(_category?.id ?? -1).then(
      (list) {
        setState(
          () {
    ...
    return new Theme(
      ...
      child: new DropdownButtonHideUnderline(
        child: new DropdownButton(
          ...
          isDense: true,
          onChanged: (value) {
            _filterByCategory(value.id).then(
              (list) {
                setState(
                  () {
                    _category = value;
  ...
  List<DropdownMenuItem<Category>> _createCatergoryDropDownMenuItems(
    ...
    List<DropdownMenuItem<Category>> menuItems = categories.map(
      (category) {
        return new DropdownMenuItem(
          value: category,
          child: new Row(
            children: <Widget>[
              new Icon(category.getIcon(), color: Colors.white, size: 20.0),
              new Padding(
  ...
  Future<List<Todo>> _filterByCategory(int categoryId) async {
    return _todoProvider.getAllTodo().then(
      (todoList) {
        switch (categoryId) {
          case -1:
            return todoList;
  ...
  List<String> _getDateList(List<Todo> todoList) {
    ...
    dates?.sort(
      (date1, date2) {
        // print(date1);
        // print(formatter.parse(date1));
        // print(date2);
        // print(formatter.parse(date2));
